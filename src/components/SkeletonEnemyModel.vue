...
<script setup lang="ts">
  ...
  const { attack, stopWalk, walk, die, idle, receiveHit, resetActions, isDead, isAttacking } =
    useCharacter(
      enemyRef,
      actions,
      {
  ...
  const soundActions = {
    attack: sounds.createAudioPlayer(
      ['skeletonEnemySwordSwing1', 'skeletonEnemySwordSwing2'],
      model
    ),
    death: sounds.createAudioPlayer(['skeletonDeath'], model, 1),
    hitReceived: sounds.createAudioPlayer(
      ['skeletonEnemyHit1', 'skeletonEnemyHit2', 'skeletonEnemyHit3'],
      model,
      1
    ),
  }
  ...
  const moveTowardsPlayer = (delta: number) => {
    ...
    if (enemyRef.value && playerState.playerPosition.value) {
      ...
      const combinedDirection = new Vector3().copy(directionToPlayer).add(separationForce)
