...
<script setup lang="ts">
  ...
  const actionSounds = {
    attack: sounds.createAudioPlayer(
      ['swordSwing1', 'swordSwing2', 'swordSwing3', 'swordSwing4'],
      model,
      0.5
    ),
    steps: sounds.createAudioPlayer(
      [
        'skeletonSteps1',
        'skeletonSteps2',
        'skeletonSteps3',
    ...
    hitReceived: sounds.createAudioPlayer(
      ['skeletonHitReceived2', 'skeletonHitReceived3', 'skeletonHitReceived4'],
      model,
      1.2
    ),
  ...
  const move = (delta) => {
    ...
    if (
      activeMovements.usingJoystick &&
      activeMovements.joystickMovement &&
      activeMovements.joystickMovement.lengthSq() > 0
    ) {
    ...
    if (isMoving) {
      ...
      if (!isFreezed) {
        ...
        playerModelRef.value.position.x = MathUtils.clamp(
          playerModelRef.value.position.x,
          -battleArenaHalfSize,
          battleArenaHalfSize
        )
        playerModelRef.value.position.z = MathUtils.clamp(
          playerModelRef.value.position.z,
          -battleArenaHalfSize,
          battleArenaHalfSize
        )
      ...
      currentQuaternion.rotateTowards(
        targetQuaternion,
        delta * (activeMovements.usingJoystick ? 8 : 10)
      )
  ...
  const attack = () => {
    ...
    const currentAttackAnimationEnum =
      Math.floor(Math.random() * 10) % 2 === 0
        ? SkeletonAnimationEnum.Sword
        : SkeletonAnimationEnum.Punch
  ...
  const doWinnerPositioning = () => {
    ...
    playerModelRef.value.quaternion.copy(
      playerQuaternion.rotateTowards(new Quaternion(0, 0, 0), 20)
    )
